    ...
    <section className="animate-fade-up space-y-6">
      ...
      <div className="space-y-8">
        {experiences.map((exp) => (
          <div
            key={exp.company}
            className="p-6 bg-white rounded-lg hover:shadow-lg transition-all duration-300 border border-gray-200"
          >
            <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-4">
              <div>
                <h3 className="text-lg font-semibold text-primary">{exp.company}</h3>
                <p className="text-dark/80 font-medium">{exp.title}</p>
              </div>
              <p className="text-sm text-dark/60">{exp.period}</p>
            </div>
            <ul className="list-disc list-inside space-y-2">
              {exp.highlights.map((highlight, index) => (
                <li key={index} className="text-sm leading-relaxed text-dark/80">
